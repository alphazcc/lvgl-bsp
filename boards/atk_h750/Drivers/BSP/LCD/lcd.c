 ...
 *
 * V1.1 20200404
 * 1, 新增lcd_fill_circle和lcd_draw_hline两个函数
 * 2, LCD初始化延时增大,避免反复开机时偶尔白屏的现象
 * 3, 修改font.h为lcdfont.h,方便兼容其他代码.
 * 4, 修改LCD_FMC_AX定义,使用移位方式,方便用户修改
 * 5, 修改lcd_read_point/lcd_draw_point和lcd_fill三个函数的颜色为32bit, 方便兼容LTDC代码
 *
 * V1.2 20200514
 * 1, 新增对ST7789 IC的支持
 * 2, 简化部分代码, 避免长判定
...
void lcd_init(void)
{
    ...
    LCD_FMC_BTRX |= 78 << 8;    /* 数据保存时间(DATAST)为78个fmc_ker_ck=4.5*78=351ns */

    /* 写时序控制寄存器 */
    LCD_FMC_BWTRX |= 0 << 28;   /* 模式A */
    LCD_FMC_BWTRX |= 15 << 0;   /* 地址建立时间(ADDSET)为15个fmc_ker_ck=67.5ns */

    /* 15个fmc_ker_ck（fmc_ker_ck=220Mhz）,某些液晶驱动IC的写信号脉宽，最少也得50ns。 */
    LCD_FMC_BWTRX |= 15 << 8;   /* 数据保存时间(DATAST)为15个fmc_ker_ck=67.5ns */

    /* 使能BANK1,区域1 */
    LCD_FMC_BCRX |= 1 << 0;     /* 使能BANK1，区域1 */
    LCD_FMC_BCRX |= (uint32_t)1 << 31; /* 使能FMC */

    lcd_opt_delay(0X1FFFFFF);   /* 初始化FMC后,必须等待一定时间才能开始初始化 */
    ...
    if (lcddev.id == 0X7789)            /* ST7789 提速 */
    {
        ...
        LCD_FMC_BWTRX |= 5 << 0;        /* 地址建立时间(ADDSET)为5个fmc_ker_ck=22.5ns */
        LCD_FMC_BWTRX |= 5 << 8;        /* 数据保存时间(DATAST)为5个fmc_ker_ck=22.5ns */
    }
    ...
    if (lcddev.id == 0X9341 || lcddev.id == 0X1963) /* 如果是这几个IC, 则设置WR时序为最快 */
    {
        ...
        LCD_FMC_BWTRX &= ~(0XF << 0);   /* 地址建立时间(ADDSET)清零 */
        LCD_FMC_BWTRX &= ~(0XF << 8);   /* 数据保存时间清零 */
        LCD_FMC_BWTRX |= 3 << 0;        /* 地址建立时间(ADDSET)为5个fmc_ker_ck=18ns */
        LCD_FMC_BWTRX |= 3 << 8;        /* 数据保存时间(DATAST)为5个fmc_ker_ck=18ns */
    }
    ...
    if (lcddev.id == 0X5510 || lcddev.id == 0X5310) /* 如果是这几个IC, 则设置WR时序为最快 */
    {
        ...
        LCD_FMC_BWTRX &= ~(0XF << 0);   /* 地址建立时间(ADDSET)清零 */
        LCD_FMC_BWTRX &= ~(0XF << 8);   /* 数据保存时间清零 */
        LCD_FMC_BWTRX |= 2 << 0;        /* 地址建立时间(ADDSET)为2个fmc_ker_ck=9ns */
        LCD_FMC_BWTRX |= 2 << 8;        /* 数据保存时间(DATAST)为2个fmc_ker_ck=9ns */
    }
